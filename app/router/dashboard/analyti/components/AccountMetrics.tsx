...
export default function TransactionHistory() {
    ...
    try {
      ...
      data.forEach((item) => {
        formattedRates[item.currency] =
          parseFloat(String(item.rateToUSD)) || 0;
      });
  ...
  // ----------------------------------------------------------------------
  // 4️⃣ CONVERSIÓN A USD
  // ----------------------------------------------------------------------
  const convertToUSD = useCallback(
    (amount: string, currencyType: string): number => {
      const value = parseFloat(amount);
      if (isNaN(value)) {
        console.warn(`Saldo no numérico encontrado: ${amount}`);
        return 0;
      }
  ...
  // ----------------------------------------------------------------------
  // 5️⃣ CÁLCULOS FINANCIEROS (useMemo)
  // ----------------------------------------------------------------------
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fadeIn">
      {/* GRÁFICO CONSOLIDADO */}
      <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 flex flex-col items-center transition-transform hover:scale-[1.02] hover:shadow-xl duration-300">
      ...
      {/* TABLA DE CUENTAS */}
      <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 transition-transform hover:scale-[1.01] hover:shadow-xl duration-300">
        <h2 className="text-lg font-semibold text-gray-800 mb-4">
          Registro de Cuentas Creadas
        </h2>

        <div className="overflow-x-auto h-[350px] scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="text-left text-gray-500 border-b bg-gray-50">
                ...
                <th className="py-2 px-2">Creada</th>
              </tr>
            </thead>
            <tbody>
              {accounts.length > 0 ? (
                accounts.map((a) => (
                  <tr
                    key={`${a.id}-${a.createAt}`}
                    className="border-b text-gray-700 hover:bg-blue-50 transition-colors"
                  >
